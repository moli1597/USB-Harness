...
| **exFAT** | ✅ 核心功能支持 | 推荐用于跨 Win/mac 大文件。npm 扁平安装无符号链接，核心可用；但 `dsh plugin` 装社区插件需 pnpm，会受限 |
| **FAT32** | ⚠️ 受限 | 单文件 ≤ 4GB 一般够用，但不支持符号链接且性能差，不推荐 |

> **为什么 exFAT 也能跑核心功能**：本项目用 **npm**（扁平化 + `.cmd` shim，无符号链接）
> 安装核心运行环境，因此绕开了 exFAT「不支持符号链接」这一主要障碍。
> 只有需要安装社区插件的 `dsh plugin` 子命令（内部用 pnpm）才要求 NTFS 或本地磁盘。
